...
const statusConfig = {
  active: "bg-leaf-50 text-leaf-700 border-leaf-200",
  soldout: "bg-amber-50 text-amber-700 border-amber-200",
  archived: "bg-soil-100 text-muted border-soil-200",
};
    ...
    <div className="p-6 max-w-4xl animate-fade-in">
      ...
      {loading ? (
        ...
        <div className="card p-16 text-center">
          ...
          <Link href="/farmer/listings/new"><Button>Create a listing</Button></Link>
        </div>
      ) : (
          ...
          {listings.map((listing) => (
            <div key={listing.id} className="card p-4 flex gap-4 items-center hover:shadow-warm-lg transition-shadow">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-display font-semibold text-earth">{listing.name}</p>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${statusConfig[listing.status] || statusConfig.archived}`}>
                  {listing.status}
                </span>
                ...
                {listing.status !== "archived" && (
                  <Button variant="ghost" size="sm" onClick={() => handleArchive(listing.id)}
